...
def extract_frames(
    ...
    """
    Extract frames from video, resizing each one to the target sprite size
    before saving. This saves disk space and speeds up the conversion phase.
    """
    ...
    # Determine and log target dimensions
    video_w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    video_h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    dummy = resize_image(Image.new("RGB", (video_w, video_h)), mode, scale_factor, custom_w, custom_h)
    target_w, target_h = dummy.size

    log_cb(
        f"Extracting frames at {target_w}×{target_h}px  ({fps:.1f} fps, {total} total)…",
        "info",
    )
    ...
    while frame_num < end:
        ...
        if (frame_num - start_frame) % frame_interval == 0:
            # Convert BGR→PIL, resize to target dimensions, then save
            pil_img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            pil_img = resize_image(pil_img, mode, scale_factor, custom_w, custom_h)
    ...
    cap.release()
    log_cb(f"Extraction complete — {saved} frames saved at {target_w}×{target_h}px.", "ok")
    done_cb(interval_ms)


def convert_image(path, palette_mode="adaptive"):
    """
    Convert a pre-resized frame PNG to a MakeCode sprite string.
    No resize step needed here — frames are already the right size.

    palette_mode:
        "adaptive" — quantise each frame to its own best 15 colours.
        "fixed"    — snap every pixel to the nearest MakeCode default palette colour.
...
def estimate_sizes(
    ...
    try:
        ...
        # Sprite dimensions (= frame dimensions, since we resize at extraction)
        ...
        # Frames disk size — now based on sprite resolution, not source resolution
        frames_cached = (
            os.path.exists(frames_dir) and bool(os.listdir(frames_dir))
        )
        if frames_cached:
            ...
            # Small sprites compress very well in PNG (~0.5 bytes/pixel)
            bytes_per_frame   = int(sprite_w * sprite_h * 0.5)
            frames_size_bytes = bytes_per_frame * frame_count

        # Output .txt size estimate
        chars_per_sprite  = 6 + sprite_h * (sprite_w + 1) + 2
        chars_per_palette = 15 * 20 + 6
        ...
        total_size_bytes = (
            (0 if frames_cached else frames_size_bytes) + output_size_bytes
        )
...
def frames_to_makecode(
    ...
    """
    Convert pre-resized frame PNGs to a MakeCode .txt file.
    No resize parameters needed — frames are already the correct size.
    """
    files = sorted(
    ...
    for i, filename in enumerate(files):
        ...
        try:
            sprite, pal = convert_image(path, palette_mode)
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for val, label, _ in [
            ("adaptive", "Adaptive  (per-frame colours, more accurate)", ""),
            ("fixed",    "Fixed  (MakeCode default palette, smaller output)", ""),
        ]:
            tk.Radiobutton(
                right, text=label,
        ...
        def est_cell(col, row, label):
            tk.Label(est_grid, text=label, font=FONT_MONO,
                     bg=CLR_PANEL, fg=CLR_DIM, anchor="w").grid(
                row=row, column=col*2, sticky="w", padx=(8, 2), pady=1)
            var = tk.StringVar(value="—")
            tk.Label(est_grid, textvariable=var,
                     font=("Courier New", 9, "bold"),
                     bg=CLR_PANEL, fg=CLR_ACCENT2, anchor="w").grid(
                row=row, column=col*2+1, sticky="w", padx=(0, 24), pady=1)
            return var
        ...
        self.est_note = tk.Label(est_outer, text="",
        ...
        self.step_label = tk.Label(bottom, text="Waiting…",
                                   font=FONT_STATUS, bg=CLR_PANEL, fg=CLR_DIM)
        ...
        self.run_btn = tk.Button(
            ...
            activebackground=CLR_ACCENT2, activeforeground=CLR_BG,
    ...
    def _path_row(self, parent, label, browse_cmd):
        ...
        tk.Button(row, text="…", font=FONT_BODY,
                  ...
                  cursor="hand2", command=browse_cmd).pack(side="left")
        return var
    ...
    def _progress(self, pct, step_text=""):
        def _do():
            ...
            self.progress_fill.config(bg=CLR_SUCCESS if pct >= 100 else CLR_ACCENT)
        self.after(0, _do)
    ...
    def _run_estimate(self):
        ...
        threading.Thread(
            ...
            args=(video_path, frames_dir, start_frame, end_frame, frame_interval,
                  mode, scale_factor, custom_w, custom_h, _done),
            daemon=True,
        ).start()
    ...
    def _show_estimate(self, r):
        ...
        if r is None:
            ...
            self.est_note.config(
                text="Could not read video. Check the path and try again.", fg=CLR_ERROR)
            return
    ...
    def _start(self):
        ...
        def after_extraction(result):
            ...
            # Phase 2: no resize args needed — frames already sized correctly
            threading.Thread(
                target=frames_to_makecode,
                args=(
                    frames_dir, output_file,
                    self._interval_ms, palette_mode,
                    self._log, self._progress, after_conversion,
                ),
                daemon=True,
            ).start()

        # Phase 1: pass resize args so frames are saved at target size
        threading.Thread(
    ...
    def _finish(self, success: bool):
        def _do():
            ...
            if success:
                ...
                self.run_btn.config(
                    state="normal", text="▶  EXTRACT  +  CONVERT", bg=CLR_SUCCESS)
            else:
                self._set_phase("✖  ERROR", CLR_ERROR)
                self.progress_fill.config(bg=CLR_ERROR)
                self.run_btn.config(
                    state="normal", text="▶  EXTRACT  +  CONVERT", bg=CLR_ACCENT)
        self.after(0, _do)
